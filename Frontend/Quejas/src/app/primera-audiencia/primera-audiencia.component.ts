...
import {
  LSTCMB_PRIMERAAUDIENCIA,
  BaseCmbClass,
  FrmProgramaAudiencia,
  FrmAudiencia,
  CmbCatalogo,
  FrmDetalle_Queja,
  CmbResultadoAudiencia,
  Frm4Registros,
  FormResponse,
  MSG_REGISTRO_DESHABILITADO,
  semana,
  data,
  formatoCitacion,
  AudienceFilterList,
...
export class PrimeraAudienciaComponent implements OnInit {
  ...
  constructor(
    ...
    private _audienciaService: AudienciaService,
    ...
    this.loc_pagina = 0;
    this.btnAudienciaVirtual = false;
      ...
      eventClick: function (info) {
        ...
        swal.fire({
          ...
          html:
            ...
            "No. Queja: <b>" +
            info.event.extendedProps.no_queja +
            "-" +
            info.event.extendedProps.año +
            "</b><br> " +
            "Descripción de Queja: <b>" +
            ...
            "</b> <br>" +
            "Audiencia: <b>" +
            info.event.extendedProps.tipoAudiencia +
            "</b> <br>" +
            "</div>",
          title:
            "<strong>No. Queja <u>" +
  ...
  validarCalendario() {
    ...
    if (today > td) {
      ...
      // console.log('data');
      // console.log(this.data);
      this._audienciaService
        .ValidarCalendario(this.data.Usuario, this.data.NoQueja, today, fin)
        .subscribe(
          (retvalue) => {
            if (retvalue["reason"] == "OK") {
  ...
  ValidadorRegistros() {
    let tempstr = null;
    this._seguridadService.ValidadorRegistros(this.data.NoQueja).subscribe(
      (Data) => {
  ...
  Save_ReProgramarAudiencia() {
    swal
      .fire({
        icon: "warning",
        title:
          "¿Está seguro que desea reprogramar esta audiencia?, ya existe otra cita previamente creada.",
  ...
  Save_Audiencia() {
    if (this.flag_Resultado) {
      if (
        confirm(
          "¿Está seguro que desea actualizar el resultado de esta audiencia?"
        )
      ) {
        this.SaveAudiencia();
      }
  ...
  LoadCmb() {
    let tempstr = "";
    this._catalogoService.getResultadoAudiencia(1).subscribe(
      (Data) => {
        if (Data["reason"] == "OK") {
          tempstr = Data["value"];
          if (tempstr != null) {
  ...
  GetQuejaList() {
    let tempstr = "";
    this._quejaService.getDataQueja(this.data.NoQueja).subscribe(
      (retvalue) => {
        if (retvalue["reason"] == "OK") {
          tempstr = retvalue["value"];
          if (tempstr != null) {
            this.lst_queja = JSON.parse(
            ...
            if (
              (this.lst_queja[0]["id_estado_queja"] >= 101 &&
                this.lst_queja[0]["id_estado_queja"] <= 130) ||
              this.lst_queja[0]["id_estado_queja"] == 151
            )
              if (this.data.Rol == 3 || this.data.Rol == 2)
                //ACTUALIZADO POR JJ
                //si es rol 3 administrador, puede editar
                /* if (this.data.Rol == 3) */ //ORIGINAL
                //EDITADO JJ   SI ES ADMIN 3 Ó SUPERVISOR 2
  ...
  LoadFrmData(isBlank: boolean) {
    console.log("JJ -> valor de isBlank: " + isBlank);
    if (!isBlank) {
      if (typeof this.lst_audiencia["0"] === "undefined") {
        ...
        if (this.lst_audiencia["0"].ProgramaAudiencia == null) {
          ...
          if (lastindex > -1) {
            ...
            if (
              this.lst_programado[lastindex].id_resultado_audiencia != null &&
              this.lst_programado[lastindex].id_resultado_audiencia != ""
            ) {
    ...
    if (flag) {
      if (typeof this.lst_audiencia["0"] === "undefined") {
        ...
        if (this.lst_audiencia["0"].ProgramaAudiencia == null) {
          ...
          this.MostrarMensaje(
            "Programa audiencia ",
            this.lst_audiencia["0"].ProgramaAudiencia
          );
        } else {
          this.lst_programado = this.lst_audiencia["0"].ProgramaAudiencia;
          this.MostrarMensaje(
            "Programa audiencia 2 ",
            this.lst_audiencia["0"].ProgramaAudiencia
          );
          let lastindex = this.lst_programado.length - 1;
  ...
  getTotalAudiencia() {
    let temp = "";
    this._audienciaService.getTotalAudiencia(this.data.NoQueja).subscribe(
      (response) => {
        if (response["value"]) {
          this.lst_audiencia = JSON.parse(
    ...
    if (today > td) {
      ...
      if (this.ProgramarForm.valid) {
        ...
        this._audienciaService.getParameter().subscribe((value) => {
          ...
          var inicio = moment(today)
            .add(-this.hour_audience, "hour")
            .format("LLL");
          ...
          var ini =
            ...
            "T" +
            moment(inicio).format("HH:mm:ss") +
            ".000Z";
          var final =
            moment(fin).format("YYYY-MM-DD") +
            "T" +
            moment(fin).format("HH:mm:ss") +
            ".000Z";
          ...
          if (today > td) {
            ...
            this._audienciaService
              .ValidarFecha(this.data.Usuario, this.data.NoQueja, ini, final)
              .subscribe(
                (retvalue) => {
                  if (retvalue["reason"] == "OK") {
                    ...
                    if (eventos == null) {
                      this._audienciaService
                        .ProgramarAudiencia(
                          ...
                          this.HoraCtrl.value,
                          this.data.NoQueja,
                          this.AudienciaCtrl.value,
                          1
                        )
                        .subscribe(
                          (retvalue) => {
                            //programación de la audiencia Atención al Consumidor
                            if (retvalue["reason"] == "OK") {
                            ...
                            } else if (this.AudienciaCtrl.value == 0) {
                              this.btnAudienciaVirtual = true;
                              //this.GenerarSegundaAudiencia() PENDIENTE TRABAJAR
                            }
                          },
                          (error) => {
                            this.flagInfoError = true;
  ...
  SaveAudiencia() {
    if (this.CerrarForm.valid) {
      ...
      this._audienciaService
        .Audiencia(this.ResultadoCtrl.value, this.data.NoQueja, 1, 1)
        .subscribe(
          (retvalue) => {
            if (retvalue["reason"] == "OK") {
              // console.log(retvalue);
  ...
  generarRegistros() {
    ...
    this._registrosservice
      .add_4Registros(this.data.NoQueja, this.id_audiencia)
      .subscribe(
        (retvalue) => {
          if (retvalue["reason"] == "OK") {
            this.flagRegError = false;
  ...
  generarRegistrosJuridico() {
    this.flagBoton2 = false;
    this._registrosservice
      .add_4RegistrosJuridico(this.data.NoQueja, this.id_audiencia)
      .subscribe(
        (retvalue) => {
          if (retvalue["reason"] == "OK") {
            this.flagRegError = false;
      ...
      if (this.linkdescription1 != "") {
        ...
        swal
          .fire({
            icon: "warning",
            title:
              "¿Está seguro que desea actualizar estos registros, con la información más reciente?",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
  ...
  GetRegistro() {
    if (this.id_audiencia != 0) {
      this._registrosservice.read_4Registros(this.id_audiencia).subscribe(
        (retvalue) => {
          if (retvalue["reason"] == "OK") {
            var tempstr = retvalue["value"];
            if (tempstr != null) {
              //console.log(retvalue);
              this.registros = JSON.parse(
                "[" + retvalue["value"].slice(0, -1) + "]"
              );
  ...
  GetRegistroJuridico() {
    if (this.id_audiencia != 0) {
      this._registrosservice
        .read_4RegistrosJuridico(this.id_audiencia)
        .subscribe(
          (retvalue) => {
            if (retvalue["reason"] == "OK") {
              var tempstr = retvalue["value"];
  ...
  Leer4Registros(blank: boolean) {
    ...
    if (blank) {
      ...
      if (typeof this.registros["0"] === "undefined") empty = true;
      if (!empty) {
        if (this.registros[0]["cedula_citacion_con_id"] != null) {
          this.tblPrimeraAudiencia = true;
          this.routerlink1 =
            "MuestraRegistro/3/" + this.registros[0]["cedula_citacion_con_id"];
          this.linkdescription1 +=
            this.registros[0]["cedula_citacion_con_codigo"];
        }
        if (this.registros[0]["cedula_citacion_pro_id"] != null) {
          this.routerlink2 =
            "MuestraRegistro/4/" + this.registros[0]["cedula_citacion_pro_id"];
          this.linkdescription2 +=
            this.registros[0]["cedula_citacion_pro_codigo"];
        }
        if (this.registros[0]["cedula_notificacion_con_id"] != null) {
          this.routerlink3 =
            "MuestraRegistro/5/" +
            this.registros[0]["cedula_notificacion_con_id"];
          this.linkdescription3 +=
            this.registros[0]["cedula_notificacion_con_codigo"];
        }
        if (this.registros[0]["cedula_notificacion_pro_id"] != null) {
          this.routerlink4 =
            "MuestraRegistro/6/" +
            this.registros[0]["cedula_notificacion_pro_id"];
          this.linkdescription4 +=
            this.registros[0]["cedula_notificacion_pro_codigo"];
        }
      }
    }
  }
    ...
    if (blank) {
      ...
      if (!empty) {
        if (this.registros[0]["cedula_citacion_jur_con_id"] != null) {
          this.tblSegundaAudiencia = true;
          this.registrosJuridico.push(
            "MuestraRegistro/3/" +
              this.registros[0]["cedula_citacion_jur_con_id"]
          );
        ...
        if (this.registros[0]["cedula_citacion_jur_pro_id"] != null) {
          this.registrosJuridico.push(
            "MuestraRegistro/4/" +
              this.registros[0]["cedula_citacion_jur_pro_id"]
          );
          // this.linkdescription2=this.registros[0]['cedula_citacion_jur_pro_codigo'];
        }

        if (
          this.registros[0]["cedula_notificacion_citacion_jur_con_id"] != null
        ) {
          this.registrosJuridico.push(
            "MuestraRegistro/4/" +
              this.registros[0]["cedula_notificacion_citacion_jur_con_id"]
          );
          // this.linkdescription3=this.registros[0]['cedula_notificacion_citacion_jur_con_codigo'];
        }
        if (
          this.registros[0]["cedula_notificacion_citacion_jur_pro_id"] != null
        ) {
          this.registrosJuridico.push(
            "MuestraRegistro/4/" +
              this.registros[0]["cedula_notificacion_citacion_jur_pro_id"]
          );
        ...
        if (this.registros[0]["cedula_notificacion_re_correo_id"] != null) {
          this.registrosJuridico.push(
            "MuestraRegistro/5/" +
              this.registros[0]["cedula_notificacion_re_correo_id"]
          );
        ...
        if (this.registros[0]["cedula_notificacion_cit_correo_id"] != null) {
          this.registrosJuridico.push(
            "MuestraRegistro/6/" +
              this.registros[0]["cedula_notificacion_cit_correo_id"]
          );
  ...
  openCedulaCitaConJur() {
    ...
    this._registrosservice
      .openCedulaCitaConJur(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
  ...
  openCedulaNotiConJur() {
    ...
    this._registrosservice
      .openCedulaNotiConJur(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaCitaProJur() {
    ...
    this._registrosservice
      .openCedulaCitaProJur(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaNotiProJur() {
    ...
    this._registrosservice
      .openCedulaNotiProJur(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaCitaNotiResCorreo() {
    ...
    this._registrosservice
      .openCedulaCitaNotiResCorreo(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaCitaNotiCitCorreo() {
    ...
    this._registrosservice
      .openCedulaCitaNotiCitCorreo(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaCitaCons() {
    ...
    this._registrosservice
      .getCedulaCitaCons(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaCitaProv() {
    ...
    this._registrosservice
      .getCedulaCitaProv(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaNotifiCons() {
    ...
    this._registrosservice
      .getCedulaNotiCons(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
  ...
  openCedulaNotifiProv() {
    ...
    this._registrosservice
      .getCedulaNotiProv(this.registros[0]["id_audiencia"])
      .subscribe(
        (Data) => {
          this._registrosservice.FileDownLoadChoose(Data, 1);
          //	this.flagDBError=false;
