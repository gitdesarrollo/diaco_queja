...
import {
  ...
  FrmRegComCon,
  FormResponse,
} from "../atencion-consumidor-const";
...
export class CatProveedorComponent implements OnInit {
  ...
  constructor(
    ...
    private _tipoComercioService: TipoComercioService,
  ...
  ngOnInit() {
    ...
    this.selectedProveedor.nombre_actividad_economica =
      "Seleccione la Actividad Economica";
    ...
    this.selectedProveedor.nombre_tipo_proveedor =
      "Seleccione el tipo Proveedor";
    this.selectedProveedor.callecilla = "Seleccione calle o avenida";

    this.asesor="";
    this.fechaFinAsesor = null;
    this.tipoProv = null;
  ...
  addOrEdit() {
    ...
    if (
      this.selectedProveedor.direccion_zona != null &&
      this.selectedProveedor.direccion != null &&
      this.selectedDepartamento.codigo_departamento != null &&
      this.selectedMunicipio.codigo_municipio != null
    ) {
      console.log("tipo proveedor");
      console.log(this.selectedProveedor.tipo_proveedor);

      if (
        this.selectedProveedor.nit_proveedor != null &&
        this.selectedProveedor.id_actividad_economica != null
      ) {
        validacion = true;
        ...
        if (this.selectedProveedor.tipo_proveedor == 7) {
          if (
            ...
            this.selectedProveedor.nombre_empresa != null &&
            this.selectedProveedor.razon_social != null
          ) {
            validacion = true;
          } else {
            validacion = false;
            alert(
              "asegurese de haber ingresado los siguientes datos: nombre de empresa, razon social y nombre proveedor"
            );
          }
        }
      } else {
        validacion = false;
        alert(
          "asegurese de haber ingresado los siguientes datos: nit, actividad economica,nombre"
        );
      }
      ...
      validacion = false;
      alert(
        "Debe ingresar los siguientes datos: direccion, zona, departamento y municipio"
      );
    }
    ...
    if (validacion) {
      if (this.selectedProveedor.id_proveedor == 0) {
        this._proveedorService
          .saveData(
            this.selectedProveedor.nombre,
            this.selectedProveedor.nit_proveedor,
            this.selectedDepartamento.codigo_departamento,
            this.selectedMunicipio.codigo_municipio,
            this.selectedProveedor.primer_nombre,
            this.selectedProveedor.segundo_nombre,
            this.selectedProveedor.tercer_nombre,
            this.selectedProveedor.primer_apellido,
            ...
            this.selectedProveedor.nombre_empresa,
            this.selectedProveedor.razon_social,
            this.selectedProveedor.email,
            this.tipoProv,
            this.selectedProveedor.telefono,
            this.selectedProveedor.id_actividad_economica,
            this.selectedComercio.id_tipo_comercio,
            this.selectedProveedor.habilitado_notificacion_electronica,
            this.selectedProveedor.servicio_publico,
            this.selectedProveedor.habilitado_conciliacion_previa,
            this.selectedProveedor.permite_calificacion,
            this.asesor,
            this.fechaFinAsesor
          )
          .subscribe(
        ...
        this._proveedorService
          .updateData(
            this.selectedProveedor.id_proveedor,
            this.selectedProveedor.nombre,
            this.selectedProveedor.nit_proveedor,
            this.selectedDepartamento.codigo_departamento,
            this.selectedMunicipio.codigo_municipio,
  ...
  actualizarMunicipio(codigo_departamento) {
    console.log(
      "this.municipioArray.length: " + this.municipioArray.length + codigo_departamento
    );
    for (let i = 1; i < this.municipioArray.length; i++) {
      console.log(
        "valor municipioArray: " + this.municipioArray[i].codigo_municipio
      );
      if (
        this.municipioArray[i].codigo_municipio ==
        this.proveedorArray[0].codigo_municipio
      ) {
        console.log(
          "ENCAJA MUNICIPIO" + this.municipioArray[i].codigo_municipio
        );
        this.selectedMunicipio = this.municipioArray[i];
      }
    }
  }
  ...
  find() {
    let tempstr = "";
    this._proveedorService
      .findByNitProveedor(this.selectedProveedor.nit_proveedor)
      .subscribe(
        (retvalue) => {
          if (retvalue["reason"] == "OK") {
            tempstr = retvalue["value"];
            if (tempstr != null) {
              this.proveedorArray = JSON.parse(
                "[" + retvalue["value"].slice(0, -1) + "]"
              );
              this.municipioObtenido = this.proveedorArray[0].codigo_municipio;
              this.GetMunicipioList(this.proveedorArray[0].codigo_departamento);
              this.selectedProveedor = this.proveedorArray[0];

              this.fechaFinAsesor = this.proveedorArray[0].fecha_fin_nombramiento;
              this.tipoProv = this.proveedorArray[0].tipo_proveedor;
              this.asesor = this.proveedorArray[0].asesor;
              this.fechaFinAsesor = this.proveedorArray[0].fecha_fin_nombramiento;

              for (let i = 1; i < this.departamentoArray.length; i++) {
                if (
                  this.departamentoArray[i].codigo_departamento ==
                  this.proveedorArray[0].codigo_departamento
                ) {
                  console.log(
                    "ENCAJA DEPARTAMENTO" +
                      this.departamentoArray[i].codigo_departamento
                  );
                  this.selectedDepartamento = this.departamentoArray[i];
                }
              }
              //this.openForEditDepartamento(this.selectedDepartamento);
              //this.GetMunicipioList(this.selectedDepartamento.codigo_departamento);

              for (let i = 1; i < this.actividadArray.length; i++) {
                if (
                  this.actividadArray[i].id_actividad_economica ==
                  this.proveedorArray[0].id_actividad_economica
                ) {
                  console.log(
                    "ENCAJA " + this.actividadArray[i].id_actividad_economica
                  );
                  this.selectedActividad = this.actividadArray[i];
                }
              }
              ...
              this.nombreDepartamento =
                this.proveedorArray[0].nombre_departamento;
              this.nombreMunicipio = this.proveedorArray[0].nombre_municipio;
              this.actividadEconomica =
                this.proveedorArray[0].nombre_actividad_economica;
              ...
              this.tipoProv = this.proveedorArray[0].tipo_proveedor;
              this.selectedProveedor.tipo_proveedor =
                this.proveedorArray[0].tipo_proveedor;
              if (
                this.proveedorArray[0].habilitado_notificacion_electronica ===
                "1"
              ) {
              ...
              if (
                this.proveedorArray[0].habilitado_conciliacion_previa === "1"
              ) {
  ...
  GetProveedorList() {
    ...
    this._proveedorService.getListProveedorComun().subscribe(
      (retvalue) => {
        if (retvalue["reason"] == "OK") {
          tempstr = retvalue["value"];
          if (tempstr != null) {
            this.proveedorComunArray = JSON.parse(
              "[" + retvalue["value"].slice(0, -1) + "]"
            );
  ...
  GetDepartamentoList() {
    ...
    this._departamentoService.getData().subscribe(
      (retvalue) => {
        if (retvalue["reason"] == "OK") {
          tempstr = retvalue["value"];
          if (tempstr != null) {
            this.departamentoArray = JSON.parse(
              "[" + retvalue["value"].slice(0, -1) + "]"
            );
            // this.departamentoArray=JSON.parse('['+retvalue["value"].slice(0, -1) +']');
  ...
  GetMunicipioList(codigoDepartamento) {
    ...
    let arr = [];
    this.municipioArray = [];
    this._municipioService.getData(codigoDepartamento).subscribe(
      (retvalue) => {
        if (retvalue["reason"] == "OK") {
          tempstr = retvalue["value"];
          if (tempstr != null) {
            console.log("Lista de departamentos obtenida.");
            this.municipioArray = JSON.parse(
              "[" + retvalue["value"].slice(0, -1) + "]"
            );
            if (this.municipioObtenido) {
              this.actualizarMunicipio(this.municipioObtenido);
            }
  ...
  GetActividadEconomicaList() {
    ...
    let arr = [];
    this.actividadArray = [];
    this._actividadEconomicaService.getListActividadEconomica().subscribe(
      (retvalue) => {
        if (retvalue["reason"] == "OK") {
          tempstr = retvalue["value"];
          if (tempstr != null) {
            this.actividadArray = JSON.parse(
              "[" + retvalue["value"].slice(0, -1) + "]"
            );
  ...
  GetTipoComercioList(idActividadEconomica) {
    ...
    let arr = [];
    this.comercioArray = [];
    this._tipoComercioService
      .getListTipoComercio(idActividadEconomica)
      .subscribe(
        (retvalue) => {
          if (retvalue["reason"] == "OK") {
            tempstr = retvalue["value"];
            if (tempstr != null) {
              this.comercioArray = JSON.parse(
                "[" + retvalue["value"].slice(0, -1) + "]"
              );
  ...
  GetProveedorComunList() {
    ...
    let arr = [];
    this.proveedorComunArray = [];
    this._proveedorComunService.getListProveedorComun().subscribe(
      (retvalue) => {
        if (retvalue["reason"] == "OK") {
          tempstr = retvalue["value"];
          if (tempstr != null) {
  ...
  openForEditTipoProveedor(event) {
    console.log("Valor nuevo de id: " + event.target.value);
    this.tipoProv = event.target.value;
    this.selectedProveedor.tipo_proveedor = event.target.value;
  ...
  openForEditActividad() {
    this.selectedProveedor.id_actividad_economica =
      this.selectedActividad.id_actividad_economica;
    this.selectedProveedor.nombre_actividad_economica =
      this.selectedActividad.nombre_actividad_economica;

    this.GetTipoComercioList(this.selectedActividad.id_actividad_economica);
  }

  openForEditDepartamento(departamento: Departamento) {
    this.selectedDepartamento = departamento;
    this.selectedProveedor.nombre_departamento =
      departamento.nombre_departamento;
    this.selectedProveedor.codigo_departamento =
  ...
  findProveedorById(idProveedor) {
    let tempstr = "";
    this._proveedorService.findById(idProveedor).subscribe(
      (retvalue) => {
        if (retvalue["reason"] == "OK") {
          tempstr = retvalue["value"];
          if (tempstr != null) {
            ...
            this.tipoProv = this.proveedorArray[0].tipo_proveedor;
            if (
              this.proveedorArray[0].habilitado_notificacion_electronica === "1"
            ) {
              this.notificacion = true;
            } else {
              this.notificacion = false;
  ...
  delete() {
    if (confirm("¿Estas seguro de eliminar el registro?")) {
      this._proveedorService
        .deleteData(this.selectedProveedor.id_proveedor)
        .subscribe(
          (retvalue) => {
            if (retvalue["reason"] == "OK") {
              console.log("Rest service success");
              this.proveedorArray = this.proveedorArray.filter(
                (x) => x != this.selectedProveedor
              );
              this.selectedProveedor = new Proveedor();
            } else {
              console.log("Rest service response ERROR.");
